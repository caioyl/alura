...
testes_sem_ex = [['D', 'A', 'B', 'C', 'A'], ['C', 'A', 'A', 'C', 'A'], ['D', 'B', 'A', 'C', 'A']]
testes_com_ex = [['D', 'A', 'B', 'C', 'A'], ['C', 'A', 'A', 'E', 'A'], ['D', 'B', 'A', 'C', 'A']]
def correcao(lista,gabarito):
  resultado = []
  for i in range(len(testes_com_ex)):
    pontuacao = 0
    for c in range(len(testes_com_ex[i])):
      if lista[i][c] in ['A','B','C','D']:
        if lista[i][c] == gabarito[c]:
          pontuacao += 1
      else:
        raise ValueError(f"A alternativa {[c+1]} da lista {i+1} não é uma opção de alternativa válida")
    resultado.append(pontuacao)
  return resultado
correcao(testes_com_ex, gabarito)
